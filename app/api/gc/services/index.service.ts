...
import { generateCategoryRankings, processGCWithRanking } from './calcRank.service';
...
export class GCService {
    ...
    async getAllGCs(month: number, year: number) {
        try {
            ...
            const gcsWithRanking = gcs.map(gc =>
            ...
            const masculineRankings = generateCategoryRankings(
                gcs.filter(gc => gc.type === 'masculine')
            );
            
            const feminineRankings = generateCategoryRankings(
                gcs.filter(gc => gc.type === 'feminine')
            );
