...
export const registerDayGC = async (
  data: ApplicationsDailysCreateManyInput,
): Promise<ApplicationsDailysCreateManyInput> => {
...
export const getAllRegistersGc = async (
  id: string,
  month: number,
  year: number,
): Promise<ApplicationsDailys[]> => {
  const response = await fetch(
    `/api/registerDay?id=${id}&month=${month}&year=${year}`,
    {
      method: "GET",
    },
  );
